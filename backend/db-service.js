...
// Conexao, carteira da vez e comparacao de texto moram em db-base.js. As
// carteiras precisam das mesmas coisas, e um require em comum e o que evita
// que os dois arquivos dependam um do outro.
const {
  supabaseAdmin,
  CARTEIRA_PADRAO,
  carteiraAtual,
...
// Reexportado no fim deste arquivo: quem chamava db-service continua chamando
// db-service, sem saber que as carteiras mudaram de casa.
...
// Seis anos. Não é limite de negócio — é o ponto em que "parcelamento" deixa
// de ser plausível e vira número errado vindo de algum lugar.
...
async function saveInstallments(phone, { description, category, installments, installmentAmount }, carteira = carteiraAtual()) {
  // A validação de verdade acontece no ai-service, na fronteira. Esta aqui
  // existe porque o banco não deve confiar em quem chama: um `installments`
  // de 100000 criava cem mil linhas, e um negativo deixava a lista vazia e
  // estourava TypeError em `linhas[0].due_month` — as duas coisas medidas.
  const quantas = Math.round(Number(installments));
  if (!Number.isFinite(quantas) || quantas < 1 || quantas > MAX_PARCELAS) {
    throw new Error(`parcelas fora da faixa: ${installments}`);
  }
...
async function apagarItem(phone, tipo, descricao, carteira = carteiraAtual()) {
  ...
  const achados = await acharPorDescricao(phone, tabela, descricao, campos, carteira);
...
module.exports = {
  CARTEIRA_PADRAO,
  planoDaMeta,
  // Carteiras vivem em carteiras.js. Sao reexportadas daqui pra que nenhum
  // chamador precise mudar: index.js e respostas.js continuam pedindo tudo ao
  // db-service, como sempre pediram.
  ...carteiras,
  comCarteira,
  carteiraAtual,
  acharPorDescricao,
